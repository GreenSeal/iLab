	...
	for(i = 0; i <= size_tokens - 1; i++) {
		...
		for(k = 0; k <= size_funcs - 1; k++){
			if(TokenTable[i] == TableFunc[k].func) {
				...
				elem.type = FUNC;
				elem.comand_value.code_func = TableFunc[k].code_func.c_str();
				
				CmdTable.push_back(elem);

				std::cout << CmdTable[j].comand_value.code_func << std::endl;
		...
		for(m = 0; m <= size_regs - 1; m++){
			...
			if(TokenTable[i] == TableReg[m].reg) {
				...
				std::cout << CmdTable[j].comand_value.elem_arg.arg.reg << std::endl;
		...
		for(k = 0; k <= size_jump - 1; k++) {
			if(TokenTable[i] == TableJump[k].jump) {
				...
				std::cout << CmdTable[j].comand_value.code_jump << " ";
		...
		if(atoi(TokenTable[i].c_str()) != 0) {
			...
			std::cout << CmdTable[j].comand_value.elem_arg.arg.arg_value << std::endl;
			continue;
		}
		...
		if(code_jump == 1) {
			struct elem_table_comands elem;
			elem.type = MARK;
			elem.comand_value.mark = TokenTable[i].c_str();

			CmdTable.push_back(elem);

			std::cout << CmdTable[j].comand_value.mark << std::endl;
