...
int CPU(std::vector<elem_table_comands> CmdTable) {
	...
	for(i = 0; i <= size_cmd - 1; i++) {
		...
		std::cout << std::endl << "0000" << std::endl << std::endl;

		if (CmdTable[i].type == FUNC) {
			
			if(CmdTable[i].comand_value.code_func[1] == '1' &&
			   CmdTable[i].comand_value.code_func[6] == '0' &&
			   CmdTable[i].comand_value.code_func[7] == '0') {
			...
			if(CmdTable[i].comand_value.code_func[1] == '1' &&
			   CmdTable[i].comand_value.code_func[6] == '0' &&
			   CmdTable[i].comand_value.code_func[7] == '1') {
			...
			if(CmdTable[i].comand_value.code_func[1] == '1' &&
			   CmdTable[i].comand_value.code_func[6] == '1' &&
			   CmdTable[i].comand_value.code_func[7] == '0') {
				assert(CmdTable[i + 1].type == ARG);
			...
			if(CmdTable[i].comand_value.code_func[1] == '0' &&
			   CmdTable[i].comand_value.code_func[6] == '0' &&
			   CmdTable[i].comand_value.code_func[7] == '0') CmdStack.Add();

			if(CmdTable[i].comand_value.code_func[1] == '0' &&
			   CmdTable[i].comand_value.code_func[6] == '0' &&
			   CmdTable[i].comand_value.code_func[7] == '1') CmdStack.Sub();
			
			if(CmdTable[i].comand_value.code_func[1] == '0' &&
			   CmdTable[i].comand_value.code_func[6] == '1' &&
			   CmdTable[i].comand_value.code_func[7] == '0') CmdStack.Mul();

			if(CmdTable[i].comand_value.code_func[1] == '0' &&
			   CmdTable[i].comand_value.code_func[6] == '1' &&
			   CmdTable[i].comand_value.code_func[7] == '1') CmdStack.Div();
